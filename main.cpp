...
void AddSchool(std::istream &istr, std::list<School> &schools) {
  ...
  // test if this achool is already in the list
  bool test = true;
  for (list<School>::iterator ite = schools.begin(); ite != schools.end(); ite++)
	  test = test && (ite->GetName() != school_name);
  if (test) // not in the list
  schools.push_back(s);
  else  // already in the list
	  cerr<<"WARNING: could not add school named "<<school_name<<", this school already exists"<<endl;
}
...
void AddStudent(std::istream &istr, std::list<Student> &students) {
  ...
  // test if this student is already in the list
  bool test = true;
  for (list<Student>::iterator ite = students.begin(); ite != students.end(); ite++)
	   test = test && (ite->GetName() != student_name);
  if (test)
	   students.push_back(s);
  else
	cerr<<"WARNING: could not add student named "<<student_name<<", this student already exists"<<endl; 
}
...
void InsertStudentIntoSchoolPreferenceList(std::istream &istr, std::list<School> &schools) {
  ...
  //
  //test if this school exists
  bool test = false;
  
  for (list<School>::iterator ite = schools.begin(); ite != schools.end(); ite++)
	  test = test || (ite->GetName() == school_name);
  if (test){
        list<School>::iterator ite = schools.begin();
		while (ite->GetName() != school_name)
		ite++;
		ite->InsertStudent(new_student_name, insert_before_student_name);
  }
  else
...
void RemoveSchoolFromStudentPreferenceList(std::istream &istr, std::list<Student> &students) {
  ...
  // FILL IN THIS FUNCTION
  //
  // test whether this student exits
  bool test = false;
  ...
  list<Student>::iterator ite = students.begin();
  while (ite->GetName() !=student_name)
	  ite++;
  ite->RemoveSchool(school_name);}
  else
...
void PerformMatching(std::list<School> &schools, std::list<Student> &students, std::ofstream &ostr) {
  ...
  while(1) {
    ...
    for (std::list<School>::iterator school_itr = schools.begin(); school_itr != schools.end(); school_itr++) {
	 ...
	 //===================================================================================
	 // modified for outputing error message
	   if (student != ""){
	 // when a school make a offer, test whether the student exists
	   bool test = false;
       for (list<Student>::iterator ite = students.begin(); ite != students.end(); ite++)
	   test = test || (ite->GetName() == student);
       if (!test) 
		   cerr<<"WARNING: could not make offer to student "<<student<<", that student does not exist"<<endl;
	   }  
	...
	if (student_itr->GetName() == student) {
	  ...
	  //ostr<<"****"<<student_itr->HasOffer()<<endl;
...
void PrintSchoolPreferenceList(std::istream &istr, const std::list<School> &schools, std::ofstream &ostr) {
  ...
  // FILL IN THIS FUNCTION
  //
  // Test whether the school is in the list before printing
  bool test = false;
  // if in the list, test will return true
  for (list<School>::const_iterator ite = schools.begin(); ite != schools.end(); ite++)
	  test = test || (ite->GetName() == school_name);
  if (test){
  for (list<School>::const_iterator ite = schools.begin(); ite != schools.end(); ite++)
  if (ite->GetName() == school_name)
  {
  ostr<<ite->GetName()<<" preference list:"<<endl;

  ite->PrintSchoolPreference(ostr);
  ostr<<endl;
  }}
  else
...
void PrintStudentPreferenceList(std::istream &istr, const std::list<Student> &students, std::ostream &ostr) {
  ...
  // FILL IN THIS FUNCTION
  //
  // test whether this student is in the list before printing
  // test will return true if in the list
  bool test = false;
  
  for (list<Student>::const_iterator ite = students.begin(); ite != students.end(); ite++)
	  test = test || (ite->GetName() == student_name);
  if (test){
  for (list<Student>::const_iterator ite = students.begin(); ite != students.end(); ite++){
  if (ite->GetName() == student_name){
  ostr<<ite->GetName()<<" preference list:"<<endl;
  ite->PrintStudentPreference(ostr);
  ostr<<endl;
  }}}
  else
